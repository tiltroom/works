    ...
    <main className="w-full">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-white/10">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Track your time, manage active sessions, and log hours.</p>
        </div>
        <LogoutButton />
      </header>
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-6 items-center justify-between">
                <div className="flex-1 w-full">
                  ...
                  <TimerCard
                    ...
                    projectName={running.projects?.name || "Unknown project"}
            ...
            <section className="glass-card rounded-2xl p-6 md:p-8 border border-zinc-800">
              ...
              <form action={startTimerAction} className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5 sm:col-span-2">
                  <label htmlFor="projectId" className="text-sm font-medium text-zinc-300">Project</label>
                  <select id="projectId" name="projectId" required className="w-full rounded-lg bg-zinc-900/50 border border-zinc-800 px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all appearance-none">
                    <option value="" className="bg-zinc-900">Select project to work on</option>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <label htmlFor="description" className="text-sm font-medium text-zinc-300">Task Description</label>
                  <input
                    id="description"
                    name="description"
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-zinc-800/60">
                    {entries.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-4 py-8 text-center text-zinc-500">No time entries recorded yet.</td>
                      </tr>
                    ) : (
                      entries.map((entry) => {
                        ...
                        return (
                          <tr key={entry.id} className={`hover:bg-zinc-800/30 transition-colors ${isRunning ? 'bg-brand-900/10' : ''}`}>
                            <td className="px-4 py-3 font-medium text-zinc-200">{entry.projects?.name || "Unknown"}</td>
                            <td className="px-4 py-3">
                              <div className="flex flex-col gap-1">
                                <span className="text-zinc-300">{new Date(entry.started_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                <span className="text-xs text-zinc-500">{new Date(entry.started_at).toLocaleDateString()}</span>
                                <span className="text-zinc-600 text-xs mt-0.5">to</span>
                                {isRunning ? (
                                  ...
                                  <span className="text-zinc-400">
                                    {entry.ended_at ? new Date(entry.ended_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : "-"}
                                  </span>
                                )}
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3 text-zinc-400 max-w-xs truncate" title={entry.description || ""}>
                              {entry.description || <span className="text-zinc-600 italic">No description</span>}
                            </td>
                          </tr>
                        );
                      })
          ...
          <section className="glass-card rounded-2xl p-6 border border-zinc-800 sticky top-6">
            ...
            <form action={createManualTimeEntryAction} className="grid gap-4">
              <div className="space-y-1.5">
                <label htmlFor="manual-projectId" className="text-sm font-medium text-zinc-300">Project</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label htmlFor="startedAt" className="text-xs font-medium text-zinc-400">Start Time</label>
                ...
                <div className="space-y-1.5">
                  <label htmlFor="endedAt" className="text-xs font-medium text-zinc-400">End Time</label>
              ...
              <div className="space-y-1.5">
                <label htmlFor="manual-description" className="text-sm font-medium text-zinc-300">Description</label>
                <textarea
